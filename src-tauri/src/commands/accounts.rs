...
use dream_core::auth::{authenticate_microsoft, check_minecraft_entitlements, is_valid_player_name, offline_uuid, AuthProgress, TokenVault};
use tauri::ipc::Channel;

fn row_to_dto(row: &rusqlite::Row) -> rusqlite::Result<AccountDto> {
    Ok(AccountDto { id: row.get(0)?, username: row.get(1)?, kind: row.get(2)?, is_active: row.get::<_, i64>(3)? != 0 })
}
...
pub fn accounts_list(state: tauri::State<'_, AppState>) -> Result<Vec<AccountDto>> {
    ...
    let mut stmt = db.prepare("SELECT id, username, kind, is_active FROM accounts ORDER BY created_at")?;
...
pub fn account_add_offline(state: tauri::State<'_, AppState>, name: String) -> Result<AccountDto> {
    if !is_valid_player_name(&name) {
        return Err(DreamError::Other(format!("недопустимое имя игрока: {name} (3-16 символов, латиница/цифры/подчёркивание)")));
    }
    let id = offline_uuid(&name);

    let db = state.db.lock().expect("db mutex poisoned");
    let existing_active: i64 = db.query_row("SELECT COUNT(*) FROM accounts WHERE is_active = 1", [], |r| r.get(0))?;
    ...
    Ok(AccountDto { id, username: name, kind: "offline".into(), is_active })
}
...
#[tauri::command]
#[specta::specta]
pub async fn account_add_microsoft(state: tauri::State<'_, AppState>, progress: Channel<AuthProgressDto>) -> Result<AccountDto> {
    ...
    let client_id = std::env::var("DREAMLAUNCHER_CLIENT_ID").unwrap_or_else(|_| "00000000-0000-0000-0000-000000000000".to_string());

    let result = authenticate_microsoft(&state.http, &client_id, |auth_progress| {
        let dto = match auth_progress {
            AuthProgress::WaitingForUser { user_code, verification_uri, expires_in } => {
                AuthProgressDto::WaitingForUser { user_code, verification_uri, expires_in: expires_in.min(u32::MAX as u64) as u32 }
            }
            ...
            AuthProgress::Complete { uuid, username } => AuthProgressDto::Complete { uuid, username },
        };
    ...
    let auth_result = match result {
        Ok(r) => r,
        Err(e) => {
            let _ = progress.send(AuthProgressDto::Error { message: e.to_string() });
            return Err(e.into());
        }
    };

    // Проверяем entitlements
    let has_license = check_minecraft_entitlements(&state.http, &auth_result.access_token).await.unwrap_or(false);
    if !has_license {
        let msg = "Аккаунт не имеет лицензии Minecraft: Java Edition".to_string();
        let _ = progress.send(AuthProgressDto::Error { message: msg.clone() });
    ...
    Ok(AccountDto { id: auth_result.uuid, username: auth_result.username, kind: "microsoft".into(), is_active })
}
...
#[tauri::command]
#[specta::specta]
pub async fn account_refresh_microsoft(state: tauri::State<'_, AppState>, account_id: String) -> Result<()> {
    let client_id = std::env::var("DREAMLAUNCHER_CLIENT_ID").unwrap_or_else(|_| "00000000-0000-0000-0000-000000000000".to_string());

    let refresh_token = TokenVault::load_refresh_token(&account_id)?.ok_or_else(|| DreamError::Other(format!("для аккаунта {account_id} нет refresh token")))?;

    let new_token = dream_core::auth::refresh_microsoft_token(&state.http, &client_id, &refresh_token).await?;
    ...
    db.execute("UPDATE accounts SET expires_at = ?1 WHERE id = ?2", rusqlite::params![expires_at.to_rfc3339(), account_id])?;
