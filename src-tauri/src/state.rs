...
impl AppState {
    pub fn init() -> dream_core::Result<Self> {
        let root = AppPaths::default_root().ok_or_else(|| {
            dream_core::CoreError::Other(
                "не удалось определить каталог данных приложения (%APPDATA%)".into(),
            )
        })?;
        let paths = AppPaths::new(root);
        paths
            .ensure_shared_dirs()
            .map_err(|e| dream_core::CoreError::io(paths.root.display().to_string(), e))?;
        let db = dream_core::storage::open(&paths.db_path())?;

        let http = reqwest::Client::builder()
            .user_agent(dream_core::mods::modrinth::user_agent(env!(
                "CARGO_PKG_VERSION"
            )))
            .build()
